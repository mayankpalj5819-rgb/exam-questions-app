...
/**
 * Detect the option format present in question text.
 * Returns "paren" for (A) (B) style, "dot" for newline A. B. style,
 * "inline" for A. text B. text on same line, or "none".
 */
function detectOptionFormat(text: string): "paren" | "dot" | "inline" | "none" {
  const normalized = normalizeText(text);

  // Pattern 1: (A) and (B) — parenthesized letters
  if (/\(A\)/.test(normalized) && /\(B\)/.test(normalized)) return "paren";

  // Pattern 2: \nA. and \nB. — letter dot at start of new line
  const lines = normalized.split("\n");
  let newlineDotCount = 0;
  for (const line of lines) {
    if (/^[A-E]\.\s/.test(line.trim())) newlineDotCount++;
  }
  if (newlineDotCount >= 2) return "dot";

  // Pattern 3a: Inline — multiple option markers on the same line
  for (const line of lines) {
    const markers = line.match(/[A-E]\.\s/g);
    if (markers && markers.length >= 2) return "inline";
  }

  // Pattern 3b: Cross-line sequential A. B. C. markers within a short span
  const allMarkers: Array<{ letter: string; pos: number; end: number }> = [];
  ...
  for (let i = 0; i < allMarkers.length; i++) {
    ...
    let count = 1;
    let expected = "B";
    for (let j = i + 1; j < allMarkers.length && count < 5; j++) {
      if (allMarkers[j].letter === expected) {
        const gap = allMarkers[j].pos - allMarkers[j - 1].end;
        if (gap < 300) {
          count++;
...
function detectQuestionType(
  ...
  // Always check text patterns first — overrides potentially wrong DB type
  if (detectOptionFormat(text) !== "none") {
    return "MCQ";
  }

  // No MCQ patterns detected — fall back to DB value or default to Numerical
...
/**
 * Clean option text by removing trailing instructions, duplicate content, and whitespace.
 * Handles scraping artifacts where the same option text is repeated multiple times.
 */
function cleanOptionText(text: string): string {
  text = text.trim();

  // Remove trailing "Choose the correct..." / "Select the correct..." text
  text = text.replace(
    /\s*(?:Choose the correct|Select the correct)[\s\S]*$/i,
    ""
  ).trim();

  // Detect repeated numeric value — scraping artifact where same option is duplicated.
  // E.g., "10mathrmm/mathrms10 \mathrm{~m} / \mathrm{s}10mathrmm/m"
  // Keep only the first occurrence when a number appears 2+ times.
  const repeatedNumMatch = text.match(/(\d+\.?\d*)([\s\S]*?)\1/);
  if (repeatedNumMatch && repeatedNumMatch.index !== undefined) {
    const firstCopyEnd =
      repeatedNumMatch.index +
      repeatedNumMatch[1].length +
      repeatedNumMatch[2].length;
    const firstCopy = text.slice(0, firstCopyEnd).trim();
    // Only apply if the first copy is substantially shorter (duplicates existed)
    if (firstCopy.length < text.length * 0.7) {
      text = firstCopy;
    }
  }
...
function extractInlineOptions(rawText: string): string[] {
  ...
  while ((match = parenRegex.exec(text)) !== null) {
    let optText = cleanOptionText(match[2]);
    if (optText.length > 500) {
      optText = optText.slice(0, 500).trim() + "…";
    }
    if (optText) parenExtracted.push(optText);
  }
  ...
  while ((match = dotRegex.exec(text)) !== null) {
    let optText = cleanOptionText(match[1]);
    if (optText.length > 500) {
      optText = optText.slice(0, 500).trim() + "…";
    }
    if (optText) dotExtracted.push(optText);
  }
  if (dotExtracted.length >= 2) return dotExtracted;

  // Strategy 3: Inline A. text B. text C. text D. text format
  // Finds a sequential A. B. C. ... marker pattern (on same line or close together)
  const allMarkers: Array<{ letter: string; pos: number; end: number }> = [];
  const markerFindRegex = /([A-E])\.\s+/g;
  let mm;
  while ((mm = markerFindRegex.exec(text)) !== null) {
    allMarkers.push({
      letter: mm[1],
      pos: mm.index,
  ...
  // Look for a sequential run starting with A: A. B. C. ... within short gaps
  for (let startIdx = 0; startIdx < allMarkers.length; startIdx++) {
    if (allMarkers[startIdx].letter !== "A") continue;

    const sequence = [allMarkers[startIdx]];
    let expectedNext = "B";

    for (
      let j = startIdx + 1;
      j < allMarkers.length && sequence.length < 5;
      j++
    ) {
      if (allMarkers[j].letter === expectedNext) {
        const prevEnd =
          sequence[sequence.length - 1].pos +
          (sequence[sequence.length - 1].end -
            sequence[sequence.length - 1].pos);
        const gap = allMarkers[j].pos - prevEnd;
        if (gap < 300) {
          // Check no instruction text between markers
          const between = text.slice(prevEnd, allMarkers[j].pos);
          if (
            !/(?:Choose the correct|Select the correct)/i.test(between)
          ) {
            sequence.push(allMarkers[j]);
            expectedNext = String.fromCharCode(
              expectedNext.charCodeAt(0) + 1
            );
          } else {
            break;
          }
      ...
      // Skip non-sequential markers
    }

    if (sequence.length >= 2) {
      const options: string[] = [];
      for (let k = 0; k < sequence.length; k++) {
        const start = sequence[k].end;
        const end =
          k + 1 < sequence.length
            ? sequence[k + 1].pos
            : Math.min(start + 500, text.length);
        let optText = cleanOptionText(text.slice(start, end));
        if (optText.length > 500) {
          optText = optText.slice(0, 500).trim() + "…";
        }
...
export function QuestionCard({
  ...
  const optionFormat = useMemo(() => {
    ...
    return detectOptionFormat(normalizedRawText);
  }, [normalizedRawText]);
  ...
  const displayText = useMemo(() => {
    if (hasInlineOptions) {
      ...
      if (optionFormat === "inline") {
        // Strip lines that contain 2+ option markers from the first marker onward
        return cleanText
          .split("\n")
          .map((line) => {
            const markers = line.match(/[A-E]\.\s/g);
            if (markers && markers.length >= 2) {
              const firstMarkerIdx = line.search(/[A-E]\.\s/);
              if (firstMarkerIdx >= 0) {
                return line.slice(0, firstMarkerIdx).trim();
              }
            }
            return line;
          })
