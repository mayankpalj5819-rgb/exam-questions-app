#!/usr/bin/env python3
"""Production scraper - scrapes all JEE PYQ chapters from ExamSide"""
...
def scrape_chapter(exam, subj, slug):
    outpath = os.path.join(DIR, f"{exam}_{subj}_{slug}.json")
    ...
    open_r = run_ab(["open", url], 40)
    if "http" not in open_r:
        return 0, False
    time.sleep(4)
    run_ab(["wait", "--load", "networkidle"], 20)
    time.sleep(2)
    
    raw = run_ab(["eval", EVAL_JS], 30)
...
def main():
    ...
    log = open("/tmp/scrape-prod.log", "a")
    ...
    for i in range(start, len(CHAPTERS)):
        ...
        count, ok = scrape_chapter(exam, subj, slug)
        total_q += count
        ...
        msg = f"[{done}/{len(CHAPTERS)}] {exam}/{subj}/{slug} → {count} questions"
        print(msg)
        log.write(msg + "\n")
        log.flush()
    ...
    run_ab(["close"])
    final = f"\n=== DONE: {done} chapters, {total_q} total questions, {empty} empty, {errors} errors ==="
    print(final)
    log.write(final + "\n")
    log.close()
